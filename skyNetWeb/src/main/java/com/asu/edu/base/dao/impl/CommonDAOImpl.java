...
public class CommonDAOImpl extends BaseDAO implements CommonDAOImplInterface {
	...
	protected Object toDataObject(ResultSet rs) throws SQLException {
		if (calledFunction.equals("getDepartments")) {
			...
			while (rs.next()) {
				...
				deptVO.setDeptDesc(rs.getString("DESC"));
				deptVO.setRootFileId(rs.getLong("ROOT_FILE_ID"));
	...
	public LinkedHashMap<Integer, DepartmentVO> getDepartments() {
		...
		LinkedHashMap<Integer, DepartmentVO> deptMap = (LinkedHashMap<Integer, DepartmentVO>) getRowByCriteria(
				sql, (Object[]) null);
	...
	public LinkedHashMap<Integer, RoleVO> getRoles() {
		...
		LinkedHashMap<Integer, RoleVO> roleMap = (LinkedHashMap<Integer, RoleVO>) getRowByCriteria(
				sql, (Object[]) null);
